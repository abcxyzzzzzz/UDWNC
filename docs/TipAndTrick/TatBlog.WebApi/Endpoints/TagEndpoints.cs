...
public static class TagEndpoints
{
    ...
    private static async Task<IResult> GetTagDetails(int id, ITagRepository tagRepository, IMapper mapper)
    {
        ...
        return tag == null 
            ...
            : Results.Ok(ApiResponse.Success(mapper.Map<AuthorItem>(tag)));
    }
    ...
    private static async Task<IResult> UpdateTag(int id, TagEditModel model, ITagRepository tagRepository, IMapper mapper)
    {
        ...
        return await tagRepository.AddOrUpdateTagAsync(tag) 
            ...
            : Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound, "Could not found tag"));
    }

    private static async Task<IResult> DeleteTag(int id, ITagRepository tagRepository)
    {
        return await tagRepository.DeleteTagByIdAsync(id) 
            ? Results.Ok(ApiResponse.Success("Tag is deleted", HttpStatusCode.NoContent))
            : Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound, $"Could not find tag with id = {id}"));
    }
}